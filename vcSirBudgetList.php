...
<?php
	...
	$result = $conn->query("SELECT * FROM demand WHERE stage = 7 and (status='seen' or status='unseen') ORDER BY id DESC LIMIT $start, $limit");
    $budgets = $result->fetch_all(MYSQLI_ASSOC);

	$result1 = $conn->query("SELECT count(id) AS id FROM demand WHERE stage = 7 and (status='seen' or status='unseen')");
...
<?php
    $sql = "UPDATE demand SET status='seen' WHERE stage=7 and status='unseen'";
    $res =  $conn->query($sql);
?>
                        ...
                        <nav aria-label="Page navigation example">
                            <ul class="pagination pagination-sm">
                                <li class="page-item <?php if($Previous == 0):?> disabled <?php endif; ?>">
                                    <a class="page-link" href="vcSirBudgetList.php?page=<?= $Previous; ?>" aria-label="Previous">
                                        <span class = "page-link" aria-hidden="true">&laquo;</span>
                                    </a>
                                </li>
                                <?php for($i = 1; $i<= $pages; $i++) : ?>
                                        <li class="page-item <?php if($i == $page):?> active <?php endif; ?>">
                                            <a class="page-link" href="vcSirBudgetList.php?page=<?= $i; ?>"> 
                                                <span class = "page-link"> <?= $i; ?> </span>
                                            </a>
                                        </li>
                                <?php endfor; ?>
                                <li class="page-item <?php if($Next == $pages+1):?> disabled <?php endif; ?>">
                                    <a class="page-link" href="vcSirBudgetList.php?page=<?= $Next; ?>" aria-label="Next">
                            ...
                            <?php 
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <a href="vcSirOpinion.php?id=<?= $budget['id'];?>">   
